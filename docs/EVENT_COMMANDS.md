...
**Character** — `setgfx`, `spawn`, `despawn`, `balloon` (emote over player/event).
...
Change Party Member/Followers, Shop/Name-Input/Save scene calls, Show Animation (battle
animation data is imported but not wired). Auto/parallel **common-event triggers** (only
CALL is wired). **N/A to this game:** vehicles, Change Tileset/Parallax/Vehicle graphics.
